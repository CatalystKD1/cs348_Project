...
function SongRow({ title, subtitle, right, onClick, songId, className = '' }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const res = await axios.get(`http://localhost:3000/user/${encodeURIComponent(user.username)}/playlists`);
  ...
  async function addToLikes() {
    try {
      await axios.post(`http://localhost:3000/users/${user.user_id}/likes/${songId}`);
      showMessage('Liked');
      setOpen(false);
    } catch (err) {
  ...
  async function addToPlaylist(playlist_id, playlist_name) {
    try {
      await axios.post(`http://localhost:3000/playlists/${playlist_id}/songs/${songId}`);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-end space-x-2">
        <div className="text-gray-400 text-right truncate mr-2">{right ?? ''}</div>

        {isAuthenticated && songId && (<div className="relative">
          <button
            aria-haspopup="true"
            aria-expanded={open}
          ...
          {open && (
            ...
            >
              <ul className="p-2">
                ...
                <li className="mt-2 border-t border-gray-700 pt-2 text-sm text-gray-300">Add to playlist</li>

                {playlists.length > 0 ? (
                    playlists.map((p) => (
                      <li key={p.playlist_id}>
                        <button
                          onClick={() => {addToPlaylist(p.playlist_id, p.playlist_name)}}
                          className="w-full text-left px-2 py-1 hover:bg-gray-700 rounded"
                        >
                          {p.playlist_name}
                        </button>
